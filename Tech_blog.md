...
每个Conda环境是独立的。如果你在不同的环境中安装相同的软件包，它们可能会被重新下载。相反，如果相同的软件包和版本已经存在于另一个环境中，且符合依赖关系，它们可能会被重用。
...
在浏览器中打开的Jupyter Notebook界面中，新建一个Notebook时，可以选择`Python (myenv)`内核，这样Notebook将在`myenv`虚拟环境中运行。
